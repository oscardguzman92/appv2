...
import {ModalController, Platform, IonSearchbar, ToastController, AlertController} from '@ionic/angular';
...
@Component({
    selector: 'app-registro-captura-ubicacion',
    templateUrl: './registro-captura-ubicacion.component.html',
    styleUrls: ['./registro-captura-ubicacion.component.scss'],
})
export class RegistroCapturaUbicacionComponent implements OnInit {
    ...
    ngOnInit() {
        this.storage.get('auth-register').then(usu => {
            this.user = JSON.parse(usu);
        }).then(() => {
            return this.storage.get('auth-user-update');
        }).then(user => {
            if (user && user.token) {
        ...
        this.responseSetShopSubscribe = this.actionsSubj
            .pipe(filter((res: ResponseSetShopAction) => res.type === RESPONSE_SET_SHOP))
            .subscribe((resOb) => {
                this.storage.get('from-register').then(res => {
                    if (!res) {
                    ...
                    this.modalController.dismiss({
                        'summary': true,
                        'summaryUser': this.user,
                        'departament': this.departament.value,
    ...
    async finish() {
        ...
        if (this.formData.valid) {
            this.user.tiendas[0].direccion = this.address.value;
            this.user.tiendas[0].descripcion_direccion = this.descriptionAdrress.value;
            this.user.tiendas[0].longitud = this.lng.value;
            this.user.tiendas[0].latitud = this.lat.value;
            this.user.tiendas[0].ciudad_id = this.city_id.value;
            this.user.tiendas[0].ciudad_nombre = this.city.value;
            this.user.slideRegister = 0;
            this.store.dispatch(new LoadingOn());
            this.storage.set('user', JSON.stringify(new UserBuilder(this.user).getUser())).then(() => {
    ...
    public get departament_id() {
        return this.formData.get('departament_id');
    }
