...
import {AFTER_SET_CLIENT_MERCHANT, AfterSetClientMerchantAction, SetClientMerchantAction} from '../../store/merchantShop.actions';
...
export class CrearClienteCapturaUbicacionComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.responseSetShopSubscribe = this.actionsSubj
            .pipe(filter((res: AfterSetClientMerchantAction) => res.type === AFTER_SET_CLIENT_MERCHANT))
            .subscribe((res: AfterSetClientMerchantAction) => {
                if(res.update){
                    this.presentAlert('Estimado mercaderista, el cliente ha sido actualizado con exito');
                    
                }else{
                    this.presentAlert('Estimado mercaderista, el cliente ha sido creado con exito');

                }
                this.navigation.goTo(this.mercaderista.rootPage, {refresh: true});
    ...
    async finish() {
        ...
        if (this.formData.valid) {
            ...
            if (this.mercaderista) {
                this.store.dispatch(new SetClientMerchantAction(
                    this.shopkeeper, this.mercaderista.mercaderista_id, this.mercaderista.token
                ));
            }
