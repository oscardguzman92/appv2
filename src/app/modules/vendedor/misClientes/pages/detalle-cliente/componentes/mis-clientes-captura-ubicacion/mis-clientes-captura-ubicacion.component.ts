...
import {
    AFTER_SET_ADDRESS_SHOP,
    AFTER_SET_CLIENT_SHOP, AfterSetAddressShopAction,
    AfterSetClientShopAction, SetAddressShopAction,
    SetClientShopAction
} from '../../../../../../tendero/registro/store/registro.actions';
...
@Component({
    selector: 'app-mis-clientes-captura-ubicacion',
    templateUrl: './mis-clientes-captura-ubicacion.component.html',
    styleUrls: ['./mis-clientes-captura-ubicacion.component.scss'],
})
export class MisClientesCapturaUbicacionComponent implements OnInit {
    @Input() shop: Shop;
    @Input() seller: Seller;
    ...
    ngOnInit() {
        ...
        this.responseSetShopSubscribe = this.actionsSubj
            .pipe(filter((res: AfterSetAddressShopAction) => res.type === AFTER_SET_ADDRESS_SHOP))
            .subscribe((res: AfterSetAddressShopAction) => {
                this.presentAlert('Se realizó la asociación de la dirección a la tienda correctamente, ' +
                    'ya puedes continuar con el pedido');
    ...
    async finish() {
        ...
        if (this.formData.valid) {
            ...
            this.shop.direccion = this.address.value;
            this.shop.descripcion_direccion = this.descriptionAdrress.value;
            this.shop.longitud = this.lng.value;
            this.shop.latitud = this.lat.value;
            this.shop.ciudad_id = this.city_id.value;
            this.shop.ciudad_nombre = this.city.value;
            this.store.dispatch(new SetAddressShopAction(this.shop, this.seller && this.seller.token));
        } else {
    ...
    private async presentAlert(msj) {
