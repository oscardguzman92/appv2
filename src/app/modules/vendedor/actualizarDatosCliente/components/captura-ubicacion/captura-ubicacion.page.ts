...
@Component({
    selector: 'app-captura-ubicacion',
    templateUrl: './captura-ubicacion.page.html',
    styleUrls: ['./captura-ubicacion.page.scss'],
})
export class CapturaUbicacionPage implements OnInit {
    @Input() shop: Shop;
    @Input() seller: Seller;
    ...
    ngOnInit() {
        const aux = (this.shop.ciu_nombre && this.shop.ciu_nombre != 'generica') ? (', ' + this.shop.ciu_nombre) : '';
        this.iniCurrentLocation();
        this.searchByAddress(this.shop.direccion + aux);
    ...
    private async presentAlert(msj) {
        ...
        if (this.seller.distribuidor && this.seller.distribuidor.telefono) {
            const obj = {
                text: 'Ir a Whatsapp',
                handler: () => {
